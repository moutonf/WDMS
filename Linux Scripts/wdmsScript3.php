...
{
	mysql_select_db($GLOBALS['dbname'],$DBConnection);
	...
	$result = mysql_query($sql, $DBConnection) or die("Error in $sql:" . mysql_error($DBConnection));	
...
{
	$con=mysql_connect($GLOBALS['ip_address'],$GLOBALS['user'],$GLOBALS['password'], $GLOBALS['dbname']);
...
function replaceFileStatus($id_name,$id_file,$id_content,$id_hash,$hash,$DBConnection)
{
		...
		if(basename($id_file) == $id_name && $hash != $id_hash)
		{
			echo "\033[0mChanged File But Not Updated: "."\033[35m".basename($id_file)."\r\n";
			setStatus($id_file,$GLOBALS['file_status2'],$DBConnection);
		}
		else
		{
